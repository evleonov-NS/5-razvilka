...
const faqs = [
  {
    ...
    a: "Структура разбора зафиксирована: сценарии, сценарий провала, дерево развилок, журнал и ревью по факту. Ответы сохраняются и сверяются с исходом — это не разовый чат, а накопление решений.",
  },
  {
    q: "Почему нет процентов вероятности?",
    a: "Проценты создают ложную точность. Мы используем только метки «низкая», «средняя» и «высокая» — этого достаточно, чтобы сравнивать сценарии без имитации прогноза.",
  },
  {
  ...
  },
  {
    ...
    a: "Описание решения — несколько минут. Генерация сценариев и сценария провала обычно укладывается в короткое ожидание на сервере; дерево развилок можно запросить отдельно.",
  },
  {
    q: "Нужно ли платить?",
    a: "Три разбора — бесплатно. Оплата в MVP не входит: после лимита добавьте свой API-ключ в настройках.",
  },
] as const;
