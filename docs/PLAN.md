...
**Следующий шаг:** прогон проверок (`build` / `llm:verify` / `db:verify`) + security-аудит — корневой [PROMPT.md](../PROMPT.md).



**Оценка до MVP (осталось):** MVP закрыт; дальше — прогон + security (вне MVP).
...
| | **Итого осталось** | | post-MVP: прогон + security (новый чат) |
