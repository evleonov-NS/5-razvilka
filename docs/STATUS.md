...
**Последний коммит:** `260b4d1` — «Социальные механики: лайки, лента /explore и демо-seed»  
...
| 6 | Журнал (главная) | 🟡 Частично | кабинет + лендинг `/` |
...
- [x] `/` — лендинг для гостей, редирект авторизованных → `/cabinet`; превью «Новые» и «Популярные» (до 10)
