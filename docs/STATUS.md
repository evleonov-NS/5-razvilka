...
**Последний коммит:** этап 7 (дерево развилок) — см. git log  
...
## Сводка (этап 7 закрыт)

| Вопрос | Ответ |
|--------|--------|
| Что умеет продукт сейчас | Войти → описать решение → 3 сценария + pre-mortem → дерево развилок (отдельная генерация) → «Что получилось?» (заглушка) |
| Чего ещё нет | Ревью / RESOLVED (8), полировка prod + ключи LLM в Vercel (9) |
| Где смотреть | Локально: `/decisions/[id]` → «Сгенерировать дерево развилок»; prod: https://5-razvilka.vercel.app |
